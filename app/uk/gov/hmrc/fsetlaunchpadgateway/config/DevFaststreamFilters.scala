...
import uk.gov.hmrc.fsetlaunchpadgateway.filters.{ FaststreamAllowlistFilter, FaststreamAuditFilter }
...
)(implicit val materializer: Materializer) extends DefaultHttpFilters(
  defaultFilters.filters.updated(defaultFilters.filters.indexWhere(_.toString.contains("AuditFilter")), auditFilter): _*
) with Logging {
  // Set to warn so we see it in prod
...
class ProductionFaststreamFilters @Inject() (
  defaultFilters: EnabledFilters,
  auditFilter: FaststreamAuditFilter,
  allowlistFilter: FaststreamAllowlistFilter)(implicit val materializer: Materializer)
